...
//썸네일 파일 삭제
$query = $db->query("select * from store_product where store_product_num = {$p_num}");
$row = $query->fetch();
...
$db->exec("update store_product set store_info_num='{$num}', store_product_name='{$product_name}', 
			store_product_origin='{$product_origin}', store_product_price='{$product_price}', 
			store_product_notice='{$product_notice}', store_product_img='{$resFile}', 
			store_product_delivery='{$product_delivery}', store_product_fee='{$product_fee}', 
			store_product_sale='{$product_sale}', store_product_regdate='{$curTime}', 
			store_product_poster='{$product_poster}',store_product_poster2='{$product_poster2}'
			where store_product_num = $p_num");


// 상품번호 받아서 돌아가기
//echo "<script>alert('상품이 등록되었습니다'); location.href=('product_form.php?num=$rwriter ');</script>";
echo "<script>alert('상품이 수정되었습니다'); location.href=('store_list.php');</script>";