...
?>          
       ...
       <body>
        ...
        <a href="recipe_write.php"><button type="submit" class="b-sel-btn" style="font-weight:bold;">레시피</button></a>
        <a href="recipe_movie.php"><button type="submit" class="b-sel-btn" style="font-weight:bold; background-color:#FF9D2D;color:#fff">영상</button></a>
         </div>
         </div>
...
$start = ($page - 1) * $numLines;
	...
	if ($sort == "regdate") {
        ...
        $query = $db->query("SELECT *,
				...
				FROM	mem_info AS b
				WHERE a.mem_info_num = b.mem_info_num) AS mem_info_nickname
			FROM recipe_movie AS a ORDER BY recipe_movie_num desc limit $start, $numLines");
	}
    
	while($row = $query->fetch()){

?>
         <li class="cell" style="width : 250px; margin:20px ">
