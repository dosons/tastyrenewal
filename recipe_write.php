...
?>          
       ...
       <body>
        ...
        <a href="recipe_write.php"><button type="submit" class="b-sel-btn" style="font-weight:bold; background-color:#FF9D2D;color:#fff">레시피</button></a>
        <a href="recipe_movie.php"><button type="submit" class="b-sel-btn" style="font-weight:bold;">영상</button></a>
         </div>
         </div>
...
$start = ($page - 1) * $numLines;
	...
	if ($sort == "regdate") {
        ...
        $query = $db->query("SELECT *,
				...
				FROM	mem_info AS b
				WHERE a.mem_info_num = b.mem_info_num) AS mem_info_nickname
			FROM recipe_info AS a ORDER BY recipe_info_num desc limit $start, $numLines");
	}
...
?>
         <li class="cell" style="width : 250px; margin:30px ">
            ...
            <div class="product-week">작성일 : <?=substr($row['recipe_info_regdate'],0,10)?></div>
        </li>
<?php
       }
?>
...
<?php
	...
	$numRecords = $query = $db->query("select count(*) from recipe_info")->fetchColumn(); 
...
?>      
         ...
         <a href="recipe_write.php?page=1&sort=<?=$sort?>" title="처음 페이지로 이동하기"><img src="img/btn-first-page.gif"></a>
         </li>
         <li class="prev pager">
         <a href="recipe_write.php?page=<?=$firstLink - $numLinks?>&sort=<?=$sort?>" title="이전 페이지로 이동하기"><img src="img/btn-prev-page.gif"></a>
         </li>
...
?>
         ...
         <li><a href="recipe_write.php?page=<?=$i?>&sort=<?=$sort?>" <?=($i == $page) ? "class='active'" : $i?> ><?=($i == $page) ? "$i" : $i?></a></li>
