...
if ($title== null){
	echo"<script>alert('게시글 제목을 입력해주세요.'); location.href=('free_update.php?num=$num');</script>";
	return;
}

if ($content == null){
	echo"<script>alert('내용을 입력해주세요.'); location.href=('free_update.php?num=$num');</script>";
	return;
}
...
$query = $db->query("SELECT *,
				...
				FROM board_info AS a where board_info_num = {$num}");
$row = $query->fetch();
...
$curTime = date("Y-m-d H:i:s");
 ...
 $db->exec("update board_info set mem_info_num='{$fnum}', board_info_title='{$title}', 
 board_info_content='{$content}', board_info_regdate='{$curTime}', board_info_file='{$fname}' 
 where board_info_num = '{$num}' ");
 


 echo "<script>alert('게시글이 수정되었습니다.'); location.href=('view.php?num=$num ');</script>";