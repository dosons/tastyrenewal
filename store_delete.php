...
$query = $db->query("select * from info_essen where store_product_num = {$num}");
$row = $query->fetch();
$e_num = $row['info_essen_num'];

$query = $db->query("select * from info_basic where store_product_num = {$num}");
$row = $query->fetch();
$b_num = $row['info_basic_num'];

$query = $db->query("select * from info_method where store_product_num = {$num}");
$row = $query->fetch();
$m_num = $row['info_method_num'];

//썸네일 파일 삭제
$query = $db->query("select * from recipe_info where store_product_num = {$num}");
$row = $query->fetch();
