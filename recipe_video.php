...
//조회수 증가
 $query = $db->query("UPDATE recipe_movie
					SET recipe_movie_hits= recipe_movie_hits+1
					WHERE  recipe_movie_num = {$movie_id};");
...
$query = $db->query("SELECT *,
	...
	from recipe_movie AS a where a.recipe_movie_num = {$movie_id}");
$row = $query->fetch();

// 작성자 제목 내용 시간 썸네일 조회수
 $r_writer = $row['mem_info_nickname'];
 $r_title = $row['recipe_movie_title'];
 $r_content = $row['recipe_movie_introduce'];
 $r_regdate = $row['recipe_movie_regdate']; 
 $movie_url = $row['recipe_movie_url']; 
 $r_hits = $row['recipe_movie_hits'];
 
 // 레시피 영상 movie_id를 받아 필수 재료 조회 쿼리
 $query = $db->query("SELECT * FROM movie_essen WHERE recipe_movie_num = {$movie_id}");
 $row = $query->fetch();

 $e_name_1 = $row['messen_name_1'];
 $e_name_2  = $row['messen_name_2'];
 $e_name_3 = $row['messen_name_3'];
 $e_name_4  = $row['messen_name_4'];

 
 $e_num_1 = $row['movie_essen_num_1'];
 $e_num_2 = $row['movie_essen_num_2'];
 $e_num_3 = $row['movie_essen_num_3'];
 $e_num_4 = $row['movie_essen_num_4'];
...
<div class="contain" style="height: auto !important;">
	...
	<div class="rec_view_top clr" style="width:1120px; height: 500px;">
		<div class="fl rec_view_img" style="height: auto !important;">
            <!-- 영상 부분 -->
            <figure class="img">
               <iframe src="https://www.youtube.com/embed/<?=$movie_url?>"
                style="width:620px; height: 400px; padding-right: 100px;" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen>
               </iframe>
            </figure>
            <!-- 영상 부분 --> 
...
<li>
<div class="b-btn01 type01" style="margin-top: -150px;">
...
<ul class="b-btn-wrap">
      ...
      <li>
         <a class="b-btn-type01" href="recipe_movie.php">레시피 영상 목록</a>
      </li>
...
.rec_content{ padding-left:90px; border-bottom:1px solid #ececec}
