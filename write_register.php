...
if ($R_title== null){
	echo"<script>alert('레시피 제목을 입력해주세요.'); location.href=('recipe_submit.php');</script>";
	return;
}

if ($R_introduce== null){
	echo"<script>alert('만드는 방법을 입력해주세요.'); location.href=('recipe_submit.php');</script>";
	return;
}
...
if($fileExt == Null) {
	echo"<script>alert('메인 이미지는 필수 항목입니다.'); location.href=('recipe_submit.php');</script>";
	return;
}
// 확장자 검사
$extStatus = false;
	...
	if($extStatus){
		...
		if($imageUpload == true){
            ...
            //업로드 성공
			 ...
			 echo "<script>alert('파일 업로드에 실패하였습니다.'); location.href=('recipe_submit.php');</script>";
		}
	}	// end if - extStatus
		// 확장자가 jpg, bmp, gif, png가 아닌 경우 else문 실행
	else {
	     echo "<script>alert('파일 확장자는 jpg, bmp, gif, png 이어야 합니다.'); location.href=('recipe_submit.php');</script>";
		
	}	
}	// end if - filetype
	// 파일 타입이 image가 아닌 경우 
else {
	echo "<script>alert('이미지 파일이 아닙니다.'); location.href=('recipe_submit.php');</script>";
...
/* recipe_write 테이블에 받아온 값 저장 
$db->exec("insert into recipe_info(recipe_info_title, recipe_info_introduce,  mem_info_num, 
			recipe_info_regdate, recipe_info_thum, recipe_info_hits)
 values('{$R_title}', '{$R_introduce}', '{$inum}', '{$curTime}', '{$resFile}', 0)");
*/

// 제목 회원번호 소개 등록일(잠깐 빼자) 썸네일(resFile) 조회수 (0시작)
$db->exec("INSERT INTO recipe_info (recipe_info_title, mem_info_num, recipe_info_introduce, recipe_info_regdate, recipe_info_thum, recipe_info_hits) 
			VALUES ('{$R_title}', '{$inum}', '{$R_introduce}','{$curTime}', '{$resFile}',0)");
...
/*$db->exec("insert into cooking_img(recipe_info_num, cooking_img_1, cooking_img_2, cooking_img_3, cooking_img_4)
...
/* 재료 등록 테이블 */ 
$db->exec("insert into info_essen(recipe_info_num, iessen_name_1, iessen_name_2, iessen_name_3, iessen_name_4, iessen_name_5, iessen_name_6,
			iessen_num_1, iessen_num_2, iessen_num_3, iessen_num_4, iessen_num_5, iessen_num_6)
 values('{$rwriter}', '{$e_name_1}', '{$e_name_2}', '{$e_name_3}', '{$e_name_4}','{$e_name_5}','{$e_name_6}',
  '{$e_num_1}', '{$e_num_2}', '{$e_num_3}', '{$e_num_4}', '{$e_num_5}','{$e_num_6}')");

$db->exec("insert into info_basic(recipe_info_num, ibasic_name_1, ibasic_name_2, ibasic_name_3, ibasic_name_4, ibasic_name_5, ibasic_name_6,
			ibasic_num_1, ibasic_num_2, ibasic_num_3, ibasic_num_4, ibasic_num_5, ibasic_num_6)
 values('{$rwriter}', '{$b_name_1}', '{$b_name_2}', '{$b_name_3}', '{$b_name_4}','{$b_name_5}','{$b_name_6}',
  '{$b_num_1}', '{$b_num_2}', '{$b_num_3}', '{$b_num_4}', '{$b_num_5}','{$b_num_6}')");
  
  
// method_img_1는 REQUEST로 받을 수 없다. multipart로 받고 위에서 넘겨받은 이미지를 처리해주고 있음
$db->exec("insert into info_method(recipe_info_num, imethod_text_1,imethod_text_2, imethod_text_3, imethod_text_4, imethod_text_5,imethod_text_6,
imethod_img_1,imethod_img_2,imethod_img_3,imethod_img_4,imethod_img_5,imethod_img_6)
 values('{$rwriter}', '{$r_txt_1}', '{$r_txt_2}', '{$r_txt_3}', '{$r_txt_4}', '{$r_txt_5}', '{$r_txt_6}',
 '{$method_img_1}','{$method_img_2}','{$method_img_3}','{$method_img_4}','{$method_img_5}','{$method_img_6}')");

/*
$query = $db->query("select * from info_basic where ibasic_name_1 = '{$i_name_1}' and ibasic_num_1 = '{$i_num_1}' and ibasic_name_2 = '{$i_name_2}' and ibasic_num_2 = '{$i_num_2}' and
ibasic_name_3 = '{$i_name_3}' and ibasic_num_3 = '{$i_num_3}' and ibasic_name_4 = '{$i_name_4}' and ibasic_num_4 = '{$i_num_4}' and 
ibasic_name_5 = '{$i_name_5}' and ibasic_num_5 = '{$i_num_5}' and ibasic_name_6 = '{$i_name_6}' and ibasic_num_6 = '{$i_num_6}' ");
$row = $query->fetch();
...
/* connection_w 테이블에 $Recipe_id 와 $Ingre_ID 값 저장 */
...
/* my_recipe테이블에 con_w_id 와 id를 저장 */
//$db->exec("insert into my_recipe(con_w_id, id) values('{$Con_ID}', '{$U_ID}')");
echo "<script>alert('레시피가 등록되었습니다.'); location.href=('recipe_view.php?num=$rwriter ');</script>";