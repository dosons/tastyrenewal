...
if ($title== null){
	echo"<script>alert('게시글 제목을 입력해주세요.'); location.href=('t_write.php');</script>";
	return;
}

if ($content== null){
	echo"<script>alert('게시글 내용을 입력해주세요.'); location.href=('t_write.php');</script>";
	return;
}
...
$type = "tip";
/* board 테이블에 받아온 값 저장 */
$db->exec("insert into board_info(board_info_title, board_info_content, board_info_writer, 
			board_info_regdate, board_info_file, board_info_hits, board_info_type)
 values('{$title}', '{$content}', '{$writer}', '{$curTime}', '{$fname}', 0, '{$type}')");
...
$Board_id = $row["board_info_num"];
 ...
 echo "<script>alert('게시글이 등록되었습니다.'); location.href=('t_view.php?num=$Board_id ');</script>";