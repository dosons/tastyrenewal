...
<div>
        <br>
       <h1 style="font-size:15px;"><b><?=$r_writer?></b>&nbsp;<na style="font-size:11px; Color:gray;"><?=substr($row['regdate'],0,16)?></na></h1>
       <br>
...
if(isset($_SESSION["userId"]) || isset($_SESSION["userName"])) {
          ...
          if($check_name == $r_writer) {
            $query3 = $db->query("select * from reply_info where board_info_num = $Board_id and reply_info_content = '{$r_content}' ");
            $row = $query3->fetch()
...
<form action="t_reply_register.php?num=<?=$Board_id?>" method="POST">
            <div id="comment_box">
...
?>
                <a href="tip.php" class="on" style="border:none;">목록</a>
                <a href="tip_update.php?num=<?=$Board_id?>">수정</a>
                <a href="tip_delete.php?num=<?=$Board_id?>">삭제</a>
