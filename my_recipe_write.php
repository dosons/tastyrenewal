...
?>          
       ...
       <body>
        ...
        <br>
        <br>
        <a href="my_recipe_write.php"><button type="submit" class="b-sel-btn" style="font-weight:bold; background-color:#FF9D2D;color:#fff">레시피</button></a>
        <a href="my_recipe_movie.php"><button type="submit" class="b-sel-btn" style="font-weight:bold;">영상</button></a>
         </div>
         </div>
    ...
    $regdate = "black";
    $hits = "black";
    if ($sort == "recipe_info_regdate") {
        $regdate = "#FF9D2D";
    } else if ($sort == "recipe_info_hits") {
        $hits = "#FF9D2D";
    }
...
$query = $db->query("select * from recipe_info where mem_info_num = '{$user_num}' ");
$row = $query->fetch();

if($row) {
//$my_connection = $row['con_w_id'];

//$query = $db->query("select * from connection_w where con_w_id = {$my_connection} ");
//$row = $query->fetch();
...
$check_name = $row['mem_info_nickname'];
	...
	if ($sort == "regdate") {
        $query = $db->query("SELECT *,
			...
			ORDER BY recipe_info_regdate desc limit $start, $numLines");
    } else if ($sort == "hits") {
        $query = $db->query("SELECT *,
				(SELECT mem_info_nickname
				FROM	mem_info AS b
			...
			FROM recipe_info AS a
			where mem_info_num = '{$user_num}'
			ORDER BY recipe_info_hits desc limit $start, $numLines");
    } else {
        $query = $db->query("SELECT *,
				(SELECT mem_info_nickname
				FROM	mem_info AS b
				WHERE a.mem_info_num = b.mem_info_num) AS mem_info_nickname
			FROM recipe_info AS a where mem_info_num = '{$user_num}' limit $start, $numLines");
	}
	
    
    //$query = $db->query("select * from recipe_info where recipe_info_writer = '{$check_name}' order by $sort desc limit $start, $numLines");
    while($row = $query->fetch()){

?>

         <li class="cell">
            <div class="img-box"><a href="recipe_view.php?num=<?=$row['recipe_info_num']?>"><img src="<?=$row['recipe_info_thum']?>" width="250" height="140" alt=""></a></div>
            <div class="product-name"><a href="recipe_view.php?num=<?=$row['recipe_info_num']?>"><?=$row['recipe_info_title']?></a></div>
            <div class="product-writer">작성자 : <?=$row['mem_info_nickname']?></div>
            <div class="product-price">조회수 :<?=$row['recipe_info_hits']?></div>
            <div class="product-week">작성일 : <?=substr($row['recipe_info_regdate'],0,16)?></div>
        </li>
<?php
       }
    } else {
?>
 <hr  style="border: solid 0.1px rgb(250, 169, 76); width: 100%;margin-bottom: 20px; margin-left: auto;margin-right: auto;">
  ...
  &emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;내가 쓴 레시피가 없습니다</a>
...
?>
      <li>
         <a class="b-btn-type01" href="recipe_submit.php" >레시피등록</a>
      </li>
...
?>
      <li>
         <a class="b-btn-type01" href="non_login.php">레시피등록</a>
      </li>
<?php      
     }
?>
...
<?php
	...
	//$numRecords = $query = $db->query("select count(*) from recipe_info where recipe_info_writer = '{$check_name}' ")->fetchColumn();
	$numRecords = $query = $db->query("select count(*) from recipe_info where mem_info_num = '{$user_num}' ")->fetchColumn();
...
?>      
         ...
         <a href="my_recipe_write.php?page=1&sort=<?=$sort?>" title="처음 페이지로 이동하기"><img src="img/btn-first-page.gif"></a>
         </li>
         <li class="prev pager">
         <a href="my_recipe_write.php?page=<?=$firstLink - $numLinks?>&sort=<?=$sort?>" title="이전 페이지로 이동하기"><img src="img/btn-prev-page.gif"></a>
         </li>
...
?>
         ...
         <li><a href="my_recipe_write.php?page=<?=$i?>&sort=<?=$sort?>" <?=($i == $page) ? "class='active'" : $i?> ><?=($i == $page) ? "$i" : $i?></a></li>
...
?>
    <li class="next pager">
    <a href="recipe_write.php?page=<?=$firstLink + $numLinks?>&sort=<?=$sort?>" title="다음 페이지로 이동하기"><img src="img/btn-next-page.gif"></a>
    </li>
    <li class="last pager">
    <a href="recipe_write.php?page=<?=$numPage ?>&sort=<?=$sort?>" title="마지막 페이지로 이동하기"><img src="img/btn-last-page.gif"></a>
    </li>
<?php
	}
?>
